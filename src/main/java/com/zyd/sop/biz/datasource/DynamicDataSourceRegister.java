package com.zyd.sop.biz.datasource;
...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        // 获取数据源类型
        String typeStr = evn.getProperty("spring.datasource.druid.master.type");
        // 获取数据源类型
        ...
        // 获取其他数据源配置
        List<Map> configs = binder.bind("spring.datasource.druid.cluster", Bindable.listOf(Map.class)).get();
        // 遍历从数据源
        for (int i = 0; i < configs.size(); i++) {
            ...
            if(consumerDatasource instanceof DruidDataSource){
                try {
                    ((DruidDataSource) consumerDatasource).setFilters("stat,wall,logback");
        ...
        if(defaultDatasource instanceof DruidDataSource){
            try {
                ((DruidDataSource) defaultDatasource).setFilters("stat,wall,logback");
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
